import { http } from './request';
import type { Article, ArticleListItem, PageResult } from '../types';

export interface ArticleListParams {
  pageNum?: number;
  pageSize?: number;
  categoryId?: number;
  keyword?: string;
}

/** 文章分页列表 GET /api/articles */
export function getArticleList(params: ArticleListParams = {}) {
  const { pageNum = 1, pageSize = 10, categoryId, keyword } = params;
  return http.get<PageResult<ArticleListItem>>('/articles', {
    pageNum,
    pageSize,
    categoryId,
    keyword,
  });
}

/** 文章详情 GET /api/articles/{id}（含 contentHtml） */
export function getArticleDetail(id: number | string) {
  return http.get<Article>(`/articles/${id}`);
}

/** 点赞 POST /api/articles/{id}/like → 返回最新点赞数 */
export function likeArticle(id: number | string) {
  return http.post<number>(`/articles/${id}/like`);
}

/** 取消点赞 DELETE /api/articles/{id}/like → 返回最新点赞数 */
export function unlikeArticle(id: number | string) {
  return http.del<number>(`/articles/${id}/like`);
}
